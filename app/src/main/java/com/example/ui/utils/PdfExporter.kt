...
object PdfExporter {
    ...
    fun generateAndShareCloudOutingPdf(
        context: Context,
        outing: CloudOuting,
        expenses: List<CloudOutingExpense>,
        currency: String
    ) {
        val totalExpenses = expenses.sumOf { it.amount }
        val participantsCount = outing.participantNames.size.coerceAtLeast(1)
        val sharePerPerson = totalExpenses / participantsCount
        ...
        paint.isFakeBoldText = true
        paint.textAlign = Paint.Align.CENTER
        canvas.drawText("الخزنة الذكية - تقرير مصاريف: ${outing.name.ifBlank { "خروجة ولمة" }}", 297f, 48f, paint)
        ...
        paint.textAlign = Paint.Align.LEFT
        val participantsSummary = if (outing.participantNames.isNotEmpty()) outing.participantNames.joinToString("، ") else "الكل"
        canvas.drawText("المشاركون: $participantsSummary", 40f, y + 30f, paint)

        y += 70f

        // Split Summary Metric Cards
        val cardWidth = 175f
        val cardHeight = 70f

        // Card 1: Total
        paint.color = Color.parseColor("#E8F5E9")
        canvas.drawRoundRect(20f, y, 20f + cardWidth, y + cardHeight, 10f, 10f, paint)
        paint.color = Color.parseColor("#2E7D32")
        paint.textSize = 11f
        paint.isFakeBoldText = true
        paint.textAlign = Paint.Align.CENTER
        canvas.drawText("إجمالي المصاريف", 20f + cardWidth / 2f, y + 25f, paint)
        paint.textSize = 16f
        canvas.drawText("${String.format(Locale.US, "%.0f", totalExpenses)} $currency", 20f + cardWidth / 2f, y + 52f, paint)

        // Card 2: Participants Count
        paint.color = Color.parseColor("#E3F2FD")
        canvas.drawRoundRect(210f, y, 210f + cardWidth, y + cardHeight, 10f, 10f, paint)
        paint.color = Color.parseColor("#1565C0")
        paint.textSize = 11f
        canvas.drawText("عدد الأفراد", 210f + cardWidth / 2f, y + 25f, paint)
        paint.textSize = 16f
        canvas.drawText("$participantsCount أفراد", 210f + cardWidth / 2f, y + 52f, paint)

        // Card 3: Per Person
        paint.color = Color.parseColor("#FFF8E1")
        canvas.drawRoundRect(400f, y, 400f + cardWidth, y + cardHeight, 10f, 10f, paint)
        paint.color = Color.parseColor("#E65100")
        paint.textSize = 11f
        canvas.drawText("حساب الفرد (النصيب)", 400f + cardWidth / 2f, y + 25f, paint)
        paint.textSize = 16f
        canvas.drawText("${String.format(Locale.US, "%.1f", sharePerPerson)} $currency", 400f + cardWidth / 2f, y + 52f, paint)
        ...
        paint.isFakeBoldText = true
        paint.textAlign = Paint.Align.RIGHT
        canvas.drawText("تفاصيل المصاريف (${expenses.size} بند):", 575f, y, paint)

        y += 15f

        // Table Header
        paint.color = Color.parseColor("#0C3B2E")
        canvas.drawRect(20f, y, 575f, y + 28f, paint)
        ...
        canvas.drawText("#", 560f, y + 18f, paint)
        canvas.drawText("بيان المصروف", 530f, y + 18f, paint)
        canvas.drawText("التصنيف", 340f, y + 18f, paint)
        canvas.drawText("التاريخ", 210f, y + 18f, paint)
        canvas.drawText("المبلغ", 80f, y + 18f, paint)

        y += 28f

        // Rows
        paint.isFakeBoldText = false
        val rowSdf = SimpleDateFormat("dd/MM/yyyy", Locale.US)
        ...
        expenses.forEachIndexed { index, exp ->
            ...
            canvas.drawText("${index + 1}", 560f, y + 16f, paint)
            val descShort = if (exp.description.length > 28) exp.description.take(26) + ".." else exp.description
            canvas.drawText(descShort, 530f, y + 16f, paint)
            canvas.drawText(exp.category.ifBlank { "عام" }, 340f, y + 16f, paint)
            canvas.drawText(rowSdf.format(Date(exp.date)), 210f, y + 16f, paint)

            paint.color = Color.parseColor("#C62828")
            paint.isFakeBoldText = true
            canvas.drawText("${String.format(Locale.US, "%.0f", exp.amount)} $currency", 80f, y + 16f, paint)
            paint.isFakeBoldText = false
        ...
        try {
            ...
            val pdfFile = File(docsDir, "Outing_${outing.name.replace(" ", "_")}_${System.currentTimeMillis()}.pdf")
            val outputStream = FileOutputStream(pdfFile)
            pdfDocument.writeTo(outputStream)
            outputStream.close()
            pdfDocument.close()
            ...
            val shareIntent = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(
                    Intent.EXTRA_TEXT,
                    "تقرير خروجة (${outing.name}) - إجمالي المصاريف: ${String.format(Locale.US, "%.0f", totalExpenses)} $currency | عدد المشاركين: $participantsCount | نصيب كل فرد: ${String.format(Locale.US, "%.1f", sharePerPerson)} $currency"
                )
                putExtra(Intent.EXTRA_SUBJECT, "تقرير مصاريف ${outing.name} - الخزنة الذكية")
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
